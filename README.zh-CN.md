...
这是轮询式观察工具，不是安全隔离机制，也不保证审计记录完整。两次轮询之间的短暂事件、可见 PipeWire 节点之外的采集，以及 watcher 停止期间的活动，都可能遗漏。`pw-dump` 单次轮询失败（超时、找不到可执行文件、非零退出码或 JSON 解析失败）会被视为"本轮状态未知"，而不是空快照——上一轮的活跃状态会原样保留，不会因此伪造 start/stop 事件，`status` 也会明确报错而不是错误地显示"当前无采集"。若两次轮询看到相同的 PipeWire 节点 ID，通常视为同一次持续采集；但 PipeWire 可能将已释放的节点 ID 重新分配给一个无关的新采集流，因此 privaudit 会用 PID 或采集类型（麦克风/摄像头）是否变化来判断该 ID 是否被回收，并据此补发对应的 stop/start 事件对——但如果两侧都没有可比较的 PID，则无法区分节点 ID 复用和真正的持续采集，此时可能不会被记录。应用活动信息敏感时，请妥善保护本地日志。
