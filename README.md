...
Alternatively, download `privaudit.pyz` from [Releases](https://github.com/zhuhroscar-tech/privaudit/releases) and run `python3 privaudit.pyz --help`. This route needs no pip installation, but still requires PipeWire and Python. Release history is tracked in [CHANGELOG.md](CHANGELOG.md).
...
This is a polling observer, not a security boundary or complete audit trail. Short events between polls, capture outside the visible PipeWire graph, and activity while the watcher is stopped may be missed. A failed `pw-dump` poll (timeout, missing binary, non-zero exit, or bad JSON) is treated as "state unknown this round," not as an empty snapshot — the previous active set carries forward and no spurious start/stop is fabricated, and `status` reports an explicit error rather than a false "no active captures." Two consecutive polls seeing the same PipeWire node id are normally treated as one continuous session; since PipeWire can reuse a freed node id for an unrelated new stream, privaudit treats a pid or capture-kind change on that id as proof of a different session and emits the stop/start pair — but if neither snapshot has a pid to compare, a same-id reuse cannot be distinguished from continuity and may go unrecorded. Protect the local log if application activity is sensitive.
